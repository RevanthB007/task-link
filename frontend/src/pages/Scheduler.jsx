...
import { Calendar, Clock, Plus, Edit3, Trash2, CheckCircle, AlertCircle, Star, User, MapPin, Zap, Settings, X } from 'lucide-react';
  ...
  const addTask = () => {
    ...
    if (currentTask) {
      setTasks(prev => prev.map(task => 
        task.id === currentTask.id ? { ...newTask, id: currentTask.id } : task
      ));
    } else {
  ...
  const generateOptimizedSchedule = async () => {
    ...
    const optimizedTasks = tasks.map(task => ({
      ...
      scheduledSlot: {
        ...
        time: `${Math.floor(Math.random() * 8) + 9}:00 AM - ${Math.floor(Math.random() * 8) + 11}:00 AM`,
        duration: task.duration || `${Math.floor(Math.random() * 3) + 1} hours (AI estimated)`,
        reason: 'Optimized based on your preferences and task priorities',
        energyMatch: task.energyLevel.charAt(0).toUpperCase() + task.energyLevel.slice(1)
      },
      status: 'scheduled',
      aiGenerated: true
    }));
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto m-4">
              <div className="flex items-center justify-between p-6 border-b">
                <h2 className="text-xl font-semibold">{currentTask ? 'Edit Task' : 'Create New Task'}</h2>
                <button
                  onClick={resetForm}
                ...
                <div className="pb-6">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={2}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                      placeholder="Tasks that must be completed first"
                    />
                  </div>
                </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border">
          <div className="px-4 py-3 border-b flex items-center justify-between">
            <h3 className="font-medium text-gray-900">
              {tasks.some(task => task.status === 'scheduled') ? 'Scheduled Tasks' : 'Tasks'}
            </h3>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center mb-2">
                          ...
                          {task.status === 'scheduled' && (
                            <span className="ml-2 px-2 py-1 bg-green-50 text-green-600 rounded-full text-xs">
                              Scheduled
                            </span>
                          )}
                        </div>
                        ...
                        {task.scheduledSlot && (
                          <div className="bg-blue-50 rounded-lg p-3 mb-3">
                            <div className="flex items-center text-sm text-blue-700 mb-1">
                              ...
                              <span className="font-medium">{task.scheduledSlot.date}</span>
                              <span className="mx-2">•</span>
                              <Clock className="w-4 h-4 mr-1" />
                              <span>{task.scheduledSlot.time}</span>
                            </div>
                            <p className="text-xs text-blue-600">{task.scheduledSlot.reason}</p>
                          </div>
                        )}
                        ...
                        <div className="text-sm text-gray-600 space-y-1">
                          ...
                          <div className="flex items-center space-x-4">
                            <span><Clock className="w-4 h-4 inline mr-1" /> {task.duration || 'To be estimated'}</span>
